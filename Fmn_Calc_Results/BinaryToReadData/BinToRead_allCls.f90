program BinaryToReadable
  ...
  integer :: num_files 
  integer, parameter :: max_values = 100 ! Maximum number of values ineach binary file
 ...
 read(*, *) num_files
  ...
  do file_num = 1, num_files
    ...
    if(file_num<10)then
    write(num_str, '(I1)') file_num
    else
    write(num_str, '(I2)') file_num
    endif
    input_file = "cls_data." // trim(num_str)
    output_file = "Data." // trim(num_str)
    ...
    num_values = num_values / (5 * 4) ! Assuming each value is of size 4bytes and there are 5 values per row
    ...
    num_values = i - 2 ! Subtract 2 to exclude the last failed read andthe initial i=1 value
    ...
    ! Deallocate arrays
    deallocate(iter, n, m, s, k)

  write(*, *) "Conversion complete!",file_num
  end do
